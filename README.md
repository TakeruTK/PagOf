...
Pensado para correr en un servidor propio (por ejemplo, un VPS) detrás de un proxy reverso con TLS (Caddy o Nginx) que termine HTTPS y reenvíe solo el tráfico de ese dominio al contenedor — el `docker-compose.yml` publica el puerto únicamente en `127.0.0.1`. Pasos:
